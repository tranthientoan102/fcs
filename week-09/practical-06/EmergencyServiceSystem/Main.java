...
public class Main {
	...
	public static int getInputAsInt(Scanner inputScan, String question) {
		int result;
		while (true){
			try {
				result = Integer.parseInt(getInput(inputScan, question));
				break;
	...
	public static boolean performAction(ServiceCenter serviceCenter, int _option, Scanner inputScan){
		...
		switch(_option){
			case 1:
				...
				int triageLevel = getInputAsInt(inputScan, "Please enter the triage level of patient: ");
			...
			case 3:
				...
				patiendId = Integer.parseInt(
						 getInput(inputScan, "Insert patient Id: "));
				serviceCenter.assignAmbulanceForPatientById(patiendId);
				waitInput(inputScan);
				break;
			case 4:
				clearScreen();
				...
				patiendId = Integer.parseInt(
						 getInput(inputScan, "Insert patient Id: "));
				int position = serviceCenter.checkPositionById(patiendId);
				if (position != -1)
					System.out.printf("Patient Id %d is at position %d in waiting list", patiendId, position);
				else
					System.out.printf("Patient Id %d is not in waiting list", patiendId);
